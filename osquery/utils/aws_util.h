...
class BatchGenerator final {
 public:
  ...
  static BatchList consumeLogDataAndGenerate(
      StringList& discarded_records,
      const std::string& log_type,
      std::vector<std::string>& log_data,
      bool append_new_lines,
      std::size_t max_bytes_per_batch,
      std::size_t max_bytes_per_record,
      std::size_t max_records_per_batch,
      std::function<void(RecordType&, Aws::Utils::ByteBuffer&)> initializer) {
    ...
    for (auto& record : log_data) {
      ...
      if (record_size >= max_bytes_per_record ||
          record_size >= max_bytes_per_batch) {
        discarded_records.push_back(std::move(record));
        continue;
      }

      if (current_batch_byte_size + record_size >= max_bytes_per_batch ||
          (current_batch.size() >= max_records_per_batch)) {
