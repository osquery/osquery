...
std::string Query::getColumnFamilyName() {
  return query_.name;
}

int Query::getInterval() {
  return query_.interval;
}
...
Status
Query::getHistoricalQueryResults(
  HistoricalQueryResults& hQR, std::shared_ptr<DBHandle> db) {
  if (isQueryNameInDatabase()) {
    std::string raw;
    auto get_status = db->Get(
      kQueries,
      query_.name,
      raw
    );
    if (get_status.ok()) {
      auto deserialize_status = deserializeHistoricalQueryResultsJSON(
        raw,
        hQR
      );
...
Status
Query::getExecutions(std::deque<int>& results, std::shared_ptr<DBHandle> db) {
