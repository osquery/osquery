...
Expected<std::string, KillswitchJSON::GetJSONError> TLSKillswitchPlugin::getJSON() {
  ...
  if(s.ok()){
    ...
    return createError(KillswitchJSON::GetJSONError::NetworkFailure,  "Could not retreive config file from network");
  }
}
} // namespace osquery
