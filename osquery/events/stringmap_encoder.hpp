...
struct StringHash {
  ...
  void add(const std::string &str) {
    if (remaining() < (str.length()+1)) { return; }
...
struct StringMapCoder {
   ...
   */
  static bool encode(const std::map<std::string,std::string> &sm, std::string &dest, StringHash &sh) {
    size_t dest_size = CalcSimpleStringMapEncodeSize(sm, sh);
   ...
   * @return true on error, false on success.
   */
  static bool decode(std::map<std::string,std::string> &sm, const std::string &encoded, uint32_t &lastKeyHash)
  {
