...
bool operator==(const WindowsFileEventSubscriptionContext& lhs,
                const WindowsFileEventSubscriptionContext& rhs) {
...
bool get_file_ref_number(const std::string& path, DWORDLONG& fileRefNumber) {
  ...
  if (INVALID_HANDLE_VALUE == h) {
    LOG(WARNING) << "unable to get handle to path " << path << ": "
                 << ::GetLastError();
    return false;
  }
  ...
  if (!::GetFileInformationByHandle(h, &info)) {
    LOG(WARNING) << "unable to get file information for " << path << ": "
                 << ::GetLastError();
    ::CloseHandle(h);
    return false;
  }
...
bool FileEventPublisher::shouldFire(
...
Status FileEventPublisher::addSubscription(
...
bool FileEventPublisher::addMonitor(const std::string& path,
                                    WindowsFileEventSubscriptionContextRef& sc,
                                    bool descend) {
  if (!pathExists(fs::path(path)).ok()) {
  ...
  if (!get_file_ref_number(path, fileRefNumber)) {
    LOG(WARNING) << "Could not get file reference number: " << path.c_str()
                 << " " << ::GetLastError();
    return false;
  }

  if (!get_file_ref_number(fs::path(path).parent_path().string(),
                           parentFileRefNumber)) {
    LOG(WARNING) << "Could not get parent file reference number: "
  ...
  if (sc->recursive && isDirectory(path).ok() && descend) {
    ...
    for (const auto& child : children) {
      auto canonicalized = fs::path(child).normalize().string();
...
void FileEventPublisher::insert_tracked_parent_dir(
...
void FileEventPublisher::insert_tracked_file(
    DWORDLONG fileRef,
    const std::string& path,
    DWORDLONG parentRef,
    const WindowsFileEventSubscriptionContextRef& sc) {
...
bool FileEventPublisher::addParentMonitor(
    const std::string& path,
    const std::string& filter,
    WindowsFileEventSubscriptionContextRef& sc) {
  if (FALSE == ::PathFileExistsA(path.c_str())) {
    LOG(WARNING) << "Path not found: " << path.c_str() << " "
                 << ::GetLastError();
    return false;
  }
...
bool FileEventPublisher::monitorSubscription(
    WindowsFileEventSubscriptionContextRef& sc) {
  std::string discovered = sc->path;

  bool monitor_parent_dir = false;
  if (discovered.back() == '/' || discovered.back() == '\\') {
    discovered += "*";
  }

  if (sc->path.find("**") != std::string::npos) {
    sc->recursive = true;
    discovered = sc->path.substr(
        0, sc->path.find("**") + 1); // keep the last * as a wildcard
    sc->path = discovered;
  }
  ...
  if (discovered.find('*') != std::string::npos) {
    ...
    auto fullpath = fs::path(discovered);
...
void FileEventPublisher::configure() {
...

  {
    ...
    for (auto& tf = tracked_files.begin(); tf != tracked_files.end();) {
      for (auto sub_iter = tf->second.subscriptions.begin();
           sub_iter != tf->second.subscriptions.end();) {
    ...
    for (auto& tp = tracked_parent_dirs.begin();
         tp != tracked_parent_dirs.end();) {
      for (auto sub_iter = tp->second.subscriptions.begin();
           sub_iter != tp->second.subscriptions.end();) {
  ...
  /* DEBUG */
  /* DEBUG */ for (auto& tf_iter : tracked_files) {
    /* DEBUG */ LOG(WARNING) << "tracking file " << tf_iter.second.path;
  /* DEBUG */}
}
...
std::string create_full_path(std::string& parent_path, USN_RECORD* record) {
  ...
  sstream << buffer;
  return fs::path(sstream.str()).normalize().string();
}
...
std::string collect_path_from_file_id(DWORDLONG fileReferenceNumber) {
  ...
  if (hint == INVALID_HANDLE_VALUE) {
    LOG(WARNING) << "Unable to get handle to local directory: "
                 << GetLastError();
    return "";
  }
  ...
  HANDLE hFile = OpenFileById(hint,
                              &id_descriptor,
                              GENERIC_READ,
                              FILE_SHARE_READ | FILE_SHARE_WRITE,
                              NULL,
  ...
  LOG(WARNING) << "Unable to get final path name for moved file: "
               << GetLastError();
  return "";
}
...
void FileEventPublisher::process_usn_record(USN_RECORD* record) {
  ...
  if (tracked_files.end() != tf_iter) {
    if (record->Reason & USN_REASON_RENAME_NEW_NAME) {
      if (record->ParentFileReferenceNumber !=
          tf_iter->second.parent_file_reference) {
        // moved to a new directory.
        // remove any old subscriptions that are directory based
        auto tp_iter =
            tracked_parent_dirs.find(tf_iter->second.parent_file_reference);
        if (tp_iter != tracked_parent_dirs.end()) {
        ...
        if (tp_iter != tracked_parent_dirs.end()) {
          std::string full_path =
              create_full_path(tp_iter->second.path, record);
          ...
          // moved out of a monitored directory into an unmonitored one, no more
          // tracking?
          ...
          auto new_path =
              collect_path_from_file_id(record->FileReferenceNumber);
          tf_iter->second.path = new_path;
        }

      } else {
        auto parent_path =
            fs::path(tf_iter->second.path).parent_path().string();
  ...
  if (record->Reason & USN_REASON_FILE_CREATE) {
    auto parent_dir_iter =
        tracked_parent_dirs.find(record->ParentFileReferenceNumber);
    if (tracked_parent_dirs.end() != parent_dir_iter) {
      std::string full_path =
          create_full_path(parent_dir_iter->second.path, record);

      bool directory = isDirectory(full_path).ok();
      // add file into to tracked files
      for (auto& sub : parent_dir_iter->second.subscriptions) {
        if (filter_matches(sub.second, record)) {
          insert_tracked_file(record->FileReferenceNumber,
                              full_path,
                              record->ParentFileReferenceNumber,
                              sub.first);
          if (sub.first->recursive && directory) {
            insert_tracked_parent_dir(record->FileReferenceNumber,
                                      full_path,
                                      sub.first,
                                      std::string("*"));
          }
...
void FileEventPublisher::buildExcludePathsSet() {
