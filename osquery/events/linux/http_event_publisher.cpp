...
void HTTPLookupEventPublisher::processPacket(unsigned char* args,
  ...
  if (getuid() == 0) {
    ...
    if (!dropper->dropTo(nobody->pw_uid, nobody->pw_gid) &&
  ...
  if (payload[0] == TLS_HANDSHAKE) {
    ...
    u_short hello_version =
        payload[OFFSET_HELLO_VERSION] * 256 + payload[OFFSET_HELLO_VERSION + 1];

    if (proto_version < SSL_MIN_GOOD_VERSION ||
        proto_version >= SSL_MAX_GOOD_VERSION ||
        hello_version < SSL_MIN_GOOD_VERSION ||
        hello_version >= SSL_MAX_GOOD_VERSION) {
    ...
    case TLS_CLIENT_HELLO: {
      ...
      if (fp_packet == nullptr) {
        fp_packet =
            static_cast<fingerprint*>(malloc(sizeof(struct fingerprint)));
        if (fp_packet == nullptr) {
      ...
      /* Extensions */
      size_t ext_len = ((size_t)packet_data[compression_pos] << 8) +
                       (size_t)packet_data[compression_pos + 1];
      ...
      for (ext_id = 0; ext_id < ext_len; ext_id++) {
        ...
        case 0x000a:
          ...
          fp_packet->curves_length =
              fp_packet->curves[2] * 256 + fp_packet->curves[3];
          break;
        case 0x000b:
          /* ec_point formats */
        ...
        case 0x000d:
          ...
          fp_packet->sig_alg_length =
              fp_packet->sig_alg[2] * 256 + fp_packet->sig_alg[3];
          break;
        case 0x0000:
        ...
        if ((packet_data + ext_id) >= (payload + size_payload)) {
          if (show_drops == 1) {
            TLOG << "Extension offset beyond end of packet "
                 << src_address_buffer << "to" << ntohs(tcp->th_sport)
                 << "destination address buffer :" << dst_address_buffer << ":"
                 << ntohs(tcp->th_dport);
          }
          return;
        }
      ...
      /*
       Extensions use offsets, etc so we can alloc those now.  Others however
       will just have pointers and we can malloc if it becomes a signature.
      */
      ...
      for (arse = 0; arse < ext_len;) {
        ...
        fp_packet->extensions[unarse + 1] =
            (uint8_t)extensions_tmp_ptr[arse + 1];
        unarse += 2;
        arse = arse + 4 + (((uint8_t)extensions_tmp_ptr[(arse + 2)]) * 256) +
               (uint8_t)(extensions_tmp_ptr[arse + 3]);
      }

      /*Construct the server name/ host */
      if (server_name != nullptr) {
        for (arse = 7;
             arse <= (size_t)(server_name[0] * 256 + server_name[1]) + 1;
             arse++) {
        ...
        if (found != std::string::npos) {
          ...
          auto host_exp =
              tryTo<std::uint64_t>(host.substr(found + 1, host.length()));
          if (host_exp) {
      ...
      if (fp_packet->extensions != nullptr) {
        ...
        for (arse = 0; arse < fp_packet->extensions_length;) {
          ...
          snprintf(buff,
                   sizeof(buff),
                   "%.02X%.02X",
                   fp_packet->extensions[arse],
                   fp_packet->extensions[arse + 1]);
          if (greaseBytes.find(buff) != greaseBytes.end()) {
            arse = arse + 2;
            continue;
          }
      ...
      if (fp_packet->curves != nullptr) {
        for (arse = 4; arse < fp_packet->curves_length + 4 &&
          ...
          snprintf(buff,
                   sizeof(buff),
                   "%.02X%.02X",
                   fp_packet->curves[arse],
                   fp_packet->curves[arse + 1]);
          if (greaseBytes.find(buff) != greaseBytes.end()) {
            continue;
          }
          unsigned int x = std::stoul(buff, nullptr, 16);
      ...
      if (fp_packet->ec_point_fmt != nullptr) {
        // Jumping to "3" to get past the second length parameter... errrr...
        // why?
      ...
      ja3 += hello_ver;
      ja3 += "," + cipherStr + "," + extensionsStr + "," + curveStr + "," +
             ecPointStr;

      // Calculate JA3 fingerprint
      ja3_fingerprint =
          hashFromBuffer(HASH_TYPE_MD5, (void*)ja3.c_str(), ja3.length());
    ...
    parser.parse(request,
                 actualHttpPacket,
                 actualHttpPacket + std::strlen(actualHttpPacket));
    ...
    while (iter != request.headers.end()) {
      std::transform(
          iter->name.begin(), iter->name.end(), iter->name.begin(), ::tolower);
      if (iter->name == "host") {
        ...
        for (auto& includeHeader :
             osquery::split(FLAGS_include_http_headers, ",")) {
          // To enforce unification we lower case all headers for matching
          std::transform(includeHeader.begin(),
                         includeHeader.end(),
                         includeHeader.begin(),
                         ::tolower);
