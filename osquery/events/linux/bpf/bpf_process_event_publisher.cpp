...
Status BPFProcessEventPublisher::run() {
  ...
  if (!rb_) {
    ...
    rb_ = std::unique_ptr<struct ::ring_buffer, void (*)(struct ::ring_buffer*)>(
...
int BPFProcessEventPublisher::handleEvent(void* ctx,
  ...
  event.comm = std::string(raw_event->comm, strnlen(raw_event->comm, TASK_COMM_LEN));
  event.path = std::string(raw_event->path, strnlen(raw_event->path, MAX_PATH_LEN));
  event.cwd = std::string(raw_event->cwd, strnlen(raw_event->cwd, MAX_PATH_LEN));
  event.args = std::string(raw_event->args, strnlen(raw_event->args, MAX_ARGS_LEN));
