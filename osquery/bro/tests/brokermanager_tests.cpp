...
namespace osquery {
    ...
    TEST_F(BrokerManagerTests, test_addandremovegroups) {
        ...
        EXPECT_TRUE(get().sendEvent(get().TOPIC_PRE_GROUPS + "test1", test_msg1).ok());
        EXPECT_TRUE(get().sendEvent(get().TOPIC_PRE_GROUPS + "test2", test_msg2).ok());
        // Receive Messages
        pollfd pfd1{get().getMessageQueue(get().TOPIC_PRE_GROUPS + "test1")->fd(), POLLIN, 0};
        pollfd pfd2{get().getMessageQueue(get().TOPIC_PRE_GROUPS + "test2")->fd(), POLLIN, 0};
        poll(&pfd1, 1, 2000);
        poll(&pfd2, 1, 2000);
        auto msgs1 = get().getMessageQueue(get().TOPIC_PRE_GROUPS + "test1")->want_pop();
        auto msgs2 = get().getMessageQueue(get().TOPIC_PRE_GROUPS + "test2")->want_pop();
        ...
        EXPECT_TRUE(get().sendEvent(get().TOPIC_PRE_GROUPS + "test1", test_msg3).ok());
        EXPECT_TRUE(get().sendEvent(get().TOPIC_PRE_GROUPS + "test2", test_msg4).ok());
        // Receive Messages
        pollfd pfd3{get().getMessageQueue(get().TOPIC_PRE_GROUPS + "test1")->fd(), POLLIN, 0};
        poll(&pfd3, 1, 2000);
        auto msgs3 = get().getMessageQueue(get().TOPIC_PRE_GROUPS + "test1")->want_pop();
        // Exactly one message expected per group
