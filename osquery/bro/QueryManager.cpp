...
namespace osquery {
    ...
    osquery::Status QueryManager::addScheduleQueryEntry(const SubscriptionRequest &qr) {
        const std::string queryID = std::to_string(this->_nextUID++);
    ...
    QueryManager::addQueryEntry(const std::string &queryID, const SubscriptionRequest &qr, const std::string& qtype) {
        ...
        if (qtype == "SCHEDULE")
            this->scheduleQueries[queryID] = ScheduleQueryEntry{queryID, query, interval, added, removed,
                                                                            snapshot};
    ...
    Status QueryManager::findQueryAndType(const std::string& queryID, std::string& qtype, std::string& query) {
