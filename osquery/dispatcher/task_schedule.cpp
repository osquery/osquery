...
void TaskSchedule::add(TaskImplementation task, UnixTime first_time_to_run) {
  task_heap_.emplace_back(std::move(task), first_time_to_run);
...
TaskSchedule::Task::Task(TaskImplementation impl, UnixTime first_time_to_run)
    : impl_(std::move(impl)), next_run_time_(first_time_to_run) {}
...
TaskSchedule::UnixTime TaskSchedule::Task::getNextTimeToRun() const noexcept {
