...
TEST_F(TokenPrivilegesTest, se_debug_privilege_guard) {
  // Store the original state of the privilege so we can verify it remains unchanged after the test
  ...
  // Create 10 more guards and check that the privilege remains enabled and the ref count is correct
  std::vector<SeDebugPrivilegeGuard> guardVector(10);
  EXPECT_EQ(getDebugTokenPrivilegeState(), SeDebugPrivState::Enabled);
  EXPECT_EQ(guard1->refCount(), 11); // guard1 + 10 in the vector

  // Destroy the guards one by one and check that the privilege remains enabled until the last guard is destroyed
  while (!guardVector.empty()) {
    guardVector.pop_back();
    EXPECT_EQ(getDebugTokenPrivilegeState(), SeDebugPrivState::Enabled);
    EXPECT_EQ(guard1->refCount(), static_cast<int>(guardVector.size()) + 1); // guard1 + remaining guards in the vector
  }

  // After all guards in the vector are destroyed, only guard1 should be active and the privilege should still be enabled
