...
namespace osquery {
namespace {
ssize_t getExtendedAttributeListSize(const std::string &path, bool user_namespace) {
  int namespace_ = user_namespace ? EXTATTR_NAMESPACE_USER : EXTATTR_NAMESPACE_SYSTEM;
...
bool getRawExtendedAttributeList(std::string &buffer, const std::string &path, bool user_namespace) {
  ...
  int namespace_ = user_namespace ? EXTATTR_NAMESPACE_USER : EXTATTR_NAMESPACE_SYSTEM;
  auto read_bytes = extattr_list_file(path.data(), namespace_, &buffer[0], buffer.size());
  if (read_bytes == -1) {
    return false;
  }
...
bool getExtendedAttributeList(std::vector<std::string> &attribute_list, const std::string &path, bool user_namespace) {
...
bool getExtendedAttribute(std::string &value, const std::string &path, bool user_namespace, const std::string &name) {
  value.clear();

  int namespace_ = user_namespace ? EXTATTR_NAMESPACE_USER : EXTATTR_NAMESPACE_SYSTEM;

  auto buffer_size = extattr_get_file(path.data(), namespace_, name.data(), nullptr, 0U);
  if (buffer_size == -1) {
    return false;
  }
  ...
  buffer_size = extattr_get_file(path.data(), namespace_, name.data(), &value[0], value.size());
  if (buffer_size == -1) {
    return false;
  }
...
bool appendExtendedAttributesForNamespace(ExtendedAttributes &attributes, const std::string &path, bool user_namespace) {
  ...
  for (const auto &attribute_name : attribute_list) {
    ...
    if (!getExtendedAttribute(attribute_value, path, user_namespace, attribute_name)) {
      return false;
    }

    attributes.push_back(std::make_pair(name_prefix + attribute_name, attribute_value));
  }
...
bool setExtendedAttributes(const std::string &path, const std::unordered_map<std::string, std::string> &attributes) {
  for (const auto &p : attributes) {
    ...
    if (extattr_set_file(path.data(), EXTATTR_NAMESPACE_USER, name.data(), value.data(), value.size()) == -1) {
      return false;
    }
  }
