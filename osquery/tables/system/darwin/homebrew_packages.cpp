...
bool getHomebrewAutoUpdate(const std::string& path) {
  ...
  //
  // Auto update data generally exists in the .metadata directory
  ...
  for (const auto& file : files) {
    ...
    if (filename == app_name + ".json") {
      return getBooleanValueFromJsonFile(file);
    }
    if (filename == app_name + ".rb") {
      return checkAutoUpdatesInFile(file);
    }
  }
  // If we didn't find the config file in the .metadata directory, we default to
  // false (should we return an error instead?)
