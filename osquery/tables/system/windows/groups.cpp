...
namespace osquery {
  ...
  namespace tables {
    ...
    std::unique_ptr<BYTE[]> GetSid(LPCWSTR accountName)
    {
      ...
      {
        LOG(WARNING) << "GetSid(): no account name provided.";
        return nullptr;
      }
      
      // Call LookupAccountNameW() once to retrieve the necessary buffer sizes for the SID (in bytes) and the domain name (in TCHARS):
      unsigned long sidBufferSize = 0;
      unsigned long domainNameSize = 0;
      auto eSidType = SidTypeUnknown;
      LookupAccountNameW(nullptr, accountName, nullptr, &sidBufferSize, nullptr, &domainNameSize, &eSidType);

      // Allocate sufficient buffers for the (binary data) SID and the (wide string) domain name:
      auto sidBuffer = std::make_unique<BYTE[]>(sidBufferSize);
      std::vector<wchar_t> domainName(domainNameSize);

      // Call LookupAccountNameW() a second time to actually obtain the SID for the given account name:
      auto ret = LookupAccountNameW(nullptr, accountName, sidBuffer.get(), &sidBufferSize, domainName.data(), &domainNameSize, &eSidType);
      if (ret == 0) {
        LOG(WARNING) << "Failed to LookupAccountNameW(): " << accountName;
      }
      else if (IsValidSid(sidBuffer.get()) == FALSE) {
        LOG(WARNING) << "The SID for " << accountName << " is invalid.";
      }

      return sidBuffer; // Implicit move operation. Caller now "owns" the returned pointer.
    }
    ...
    void processLocalGroups(QueryData& results) {
      ...
      do {
        ret = NetLocalGroupEnum(nullptr, groupInfoLevel, (LPBYTE *)&lginfo, MAX_PREFERRED_LENGTH, &numGroupsRead, &totalGroups, nullptr);
        ...
        if ((ret == NERR_Success || ret == ERROR_MORE_DATA) && lginfo != nullptr) {
          ...
          LOG(WARNING) << "NetLocalGroupEnum failed with return value: " << ret;
        }
        if (lginfo != nullptr) {
