...
Status getObjectName(const NtQueryObject &_NtQueryObject,
  ...
  if (_NtQueryObject(processDupHandle, ObjectNameInformation, &pName,
                     sizeof(pName), &returnLength) != STATUS_SUCCESS) {
      ...
      *szDrive = *p;
      ...
      if (QueryDosDevice(szDrive, szName, MAX_PATH)) {
        ...
        if (uNameLen < MAX_PATH) {
          ...
          if (bFound) {
            filename = wstringToString(szDrive) +
                       wstringToString(pszFilename + uNameLen);
          }
        }
      }
...
BOOL getObjectType(const NtQueryObject &_NtQueryObject,
                   const HANDLE &processDupHandle,
                   PUBLIC_OBJECT_TYPE_INFORMATION *objectTypeInfo) {
  return (_NtQueryObject(processDupHandle, ObjectTypeInformation,
                         objectTypeInfo, BUFF_SIZE, NULL) == STATUS_SUCCESS);
}

Status getHandleInfo(const HANDLE &handle, const NtQueryObject &_NtQueryObject,
                     ObjectInfoTuple &objInfo) {
  std::string objectName;
  PPUBLIC_OBJECT_TYPE_INFORMATION objectTypeInfo;

  if ((objectTypeInfo = (PPUBLIC_OBJECT_TYPE_INFORMATION)malloc(BUFF_SIZE)) ==
      NULL) {
  ...
  return Status::failure("Could not get object name for " +
                         std::get<0>(objInfo));
}
...
Status getSystemHandles(PSYSTEM_HANDLE_INFORMATION_EX &handleInfo) {
  ...
  while ((ULONG)(ntstatus = _NtQuerySystemInformation(
    ...
    if ((handleInfo = (PSYSTEM_HANDLE_INFORMATION_EX)realloc(
             handleInfo, initialAllocationBuffer *= 2)) == NULL) {
      return Status(GetLastError(),
                    "Could not re-allocate memory for for handleInfo");
    }
  }
...
std::string getHandleAttributes(const ULONG &handleAttributes) {
  ...
  if ((handleAttributes & OBJ_IGNORE_IMPERSONATED_DEVICEMAP) ==
...
QueryData genHandles(QueryContext &context) {
  ...
  for (i = 0; i < handleInfo->NumberOfHandles; i++) {
    ...
    auto const guard_process_handle =
        scope_guard::create([&processHandle]() { CloseHandle(processHandle); });

    if (_NtDuplicateObject(processHandle, (HANDLE)(intptr_t)handle.HandleValue,
                           GetCurrentProcess(), &processDupHandle, GENERIC_READ,
                           0, 0) != STATUS_SUCCESS) {
    ...
    ObjectInfoTuple objInfo;
    auto status_hinfo =
        getHandleInfo(processDupHandle, _NtQueryObject, objInfo);
    if (!status_hinfo.ok()) {
