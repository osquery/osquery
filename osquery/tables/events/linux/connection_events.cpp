 ...
 *
 * This subscriber retrieves events from the ConntrackEventPublisher about status changes
 * of network flows. Flows are defined by source and destination among the ISO/OSI layer 3 and 4.
 */
class ConnectionEventSubscriber : public EventSubscriber<ConntrackEventPublisher> {
 public:
...
Status ConnectionEventSubscriber::Callback(const ECRef& ec, const SCRef& sc) {
  ...
  // Layer 4 - UDP and TCP
  r["protocol"] = INTEGER( (int) nfct_get_attr_u8(ec->event.get(), ATTR_ORIG_L4PROTO));
  ...
  switch (src_addr.ss_family) {
    case AF_INET: {
      ...
      r["orig_address"] = std::string{inet_ntoa(((struct sockaddr_in *) &src_addr)->sin_addr)};
      r["resp_address"] = std::string{inet_ntoa(((struct sockaddr_in *) &dst_addr)->sin_addr)};
    ...
    case AF_INET6: {
      ...
      memcpy(&((struct sockaddr_in6 *) &src_addr)->sin6_addr.s6_addr32, &ipv6.src, sizeof(ipv6.src));
      memcpy(&((struct sockaddr_in6 *) &dst_addr)->sin6_addr.s6_addr32, &ipv6.dst, sizeof(ipv6.dst));

      char straddr[INET6_ADDRSTRLEN];
      inet_ntop( AF_INET6, &((struct sockaddr_in6 *) &src_addr)->sin6_addr, straddr, sizeof(straddr) );
      r["orig_address"] = std::string{straddr};
      inet_ntop( AF_INET6, &((struct sockaddr_in6 *) &dst_addr)->sin6_addr, straddr, sizeof(straddr) );
    ...
    default:
      LOG(WARNING) << "Unsupported level 3 protocol number: " + std::to_string(src_addr.ss_family);
  ...
  for (osquery::Row &result: results) {
    if ( (result["local_address"] == r["orig_address"] && result["remote_address"] == r["resp_address"] && result["protocol"] == r["protocol"] && result["local_port"] == r["orig_port"] && result["remote_port"] == r["resp_port"]) || (result["local_address"] == r["resp_address"] && result["remote_address"] == r["orig_address"] && result["protocol"] == r["protocol"] && result["local_port"] == r["resp_port"] && result["remote_port"] == r["orig_port"]) ) {
  ...
  // Find the process to the respective inode - Search among the entries in /proc/<pid>/fd
  inodes.clear();
