...
*/
static int hashCreate(sqlite3 *db, /* Database where module is created */
                      void *pAux, /* clientdata for the module */
                      int argc, /* Number of arguments */
                      const char *const *argv, /* Value for all arguments */
                      sqlite3_vtab **ppVtab, /* Write the new virtual table
                                                object here */
...
static int hashColumn(sqlite3_vtab_cursor *cur, sqlite3_context *ctx, int col) {
  ...
  if (pCur->row >= 0 && pCur->row < pVtab->pContent->n) {
    ...
    case 1:
      sqlite3_result_text(
          ctx, (pVtab->pContent->filename[pCur->row]).c_str(), -1, nullptr);
      break;
    // md5
    case 2:
      sqlite3_result_text(
          ctx, (pVtab->pContent->md5[pCur->row]).c_str(), -1, nullptr);
      break;
    }
  }
  return SQLITE_OK;
 ...
 * an error occurs.
**/
static int hashFilter(sqlite3_vtab_cursor *pVtabCursor,
                      int idxNum,
                      const char *idxStr,
                      int argc,
                      sqlite3_value **argv) {
...
int sqlite3_hash_create(sqlite3 *db,
  ...
  *ppReturn = p = new sqlite3_hash;
  ...
  rc = sqlite3_create_module_v2(
      db, zName, &hashModule, p, (void (*)(void *))hashFree);
  if (rc == SQLITE_OK) {
    char *zSql;
    zSql =
