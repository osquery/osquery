...
QueryData genContainerStats(QueryContext& context) {
  ...
  for (const auto& id : context.constraints["id"].getAll(EQUALS)) {
    ...
    try {
      ...
      r["memory_inactive_file"] =
          BIGINT(container.get<uint64_t>("memory_stats.stats.inactive_file", 0));
      r["memory_total_inactive_file"] =
          BIGINT(container.get<uint64_t>("memory_stats.stats.total_inactive_file", 0));
      r["memory_max_usage"] =
