...
std::map<std::string, std::string> dkFieldMappings = {
    {"Package", "name"},
    {"Version", "version"},
    {"Installed-Size", "size"},
    {"Architecture", "arch"},
...
void extractDebPackageInfo(const struct pkginfo* pkg,
  ...
  for (fip = dfieldinfos; fip->name; fip++) {
    ...
    if (!line.empty()) {
      ...
      auto it = dkFieldMappings.find(key);
      if (it != dkFieldMappings.end()) {
...
QueryData genDockerDebPackages(QueryContext& context) {
  ...
  for (const auto& id : context.constraints["id"].getAll(EQUALS)) {
    ...
    if (!s.ok()) {
      VLOG(1) << "Error getting docker container info " << id << ": " << s.what();
      continue;
    }
    ...
    if (fd == -1) {
      VLOG(1) << "unable to open mnt ns for pid " << pid << " : " << strerror(errno);
      continue;
    }
