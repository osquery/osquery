...
Row parse_diag_msg(struct inet_diag_msg *diag_msg, int rtalen){
    ...
    if(diag_msg->idiag_family == AF_INET){
        ...
        throw "Unknown family";
    }

    if(local_addr_buf[0] == 0 || remote_addr_buf[0] == 0){
        throw "Could not get required connection information";
    } else {
        Row row;
        row["inode"] =  boost::lexical_cast<std::string> ( diag_msg->idiag_inode );
        row["local_port"] = boost::lexical_cast<std::string> ( ntohs(diag_msg->id.idiag_sport) );
        row["remote_port"] = boost::lexical_cast<std::string> ( ntohs(diag_msg->id.idiag_dport) );
