...
If you are developing latent tables that would be blacklisted, please make sure you are relying on a feature with a clear issue and traction. Then add your table name (as it appears in the `.table` spec) to [`specs/blacklist`](https://github.com/osquery/osquery/blob/master/specs/blacklist) and define `DISABLE_BLACKLIST=1` in your configure step when building.
