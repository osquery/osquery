...
Status logs are generated by the [Glog logging framework](https://github.com/google/glog/). By default GLOG these logs to disk. Logger plugins may also write status logs and write them to system or otherwise.
...
Note: In the LaunchDaemon, systemd, and initscript provided in the osquery packages, the minimum stderr reporting is limited to `WARNING` to help minify the content duplicated to syslog.
