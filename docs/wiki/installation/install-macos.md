...
Continuous Integration currently tests macOS builds of osquery against macOS 26 and macOS 15 for both x86_64 and arm64 architectures (see the `build_macos` section of the [hosted runners CI configuration](https://github.com/osquery/osquery/blob/master/.github/workflows/hosted_runners.yml)). All core functionality of osquery should work on macOS 13.3 or newer, although some tables may read data present only on certain versions of macOS, as Apple adds new data sources or deprecates others. Versions of macOS listed as out of support on [endoflife.date/macos](https://endoflife.date/macos) are not supported.
