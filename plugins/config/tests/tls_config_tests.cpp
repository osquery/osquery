...
class TLSConfigTests : public testing::Test {
 ...
 protected:
  std::unique_ptr<TLSConfigPlugin> createETagPlugin(bool enabled = true,
                                                    bool node_api = false) {
    // The server's periodic node-key invalidation would inject
    // re-enrollment retries into the recorded event sequences these
    // tests assert on; suppress it explicitly for this server.
    JSON reset;
    reset.addCopy("enroll_invalidation", "off");
    postToTestServer("/reset_config_test_state", reset);
