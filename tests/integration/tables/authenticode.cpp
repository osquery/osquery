...
TEST_F(authenticode, test_sanity) {
  ...
  auto const windir = getEnvVar("WINDIR");
  ASSERT_TRUE(windir.is_initialized());

  auto const query = std::string("select * from authenticode where path = '") +
                     *windir + "\\System32\\notepad.exe'";
  auto const data = execute_query(query);
