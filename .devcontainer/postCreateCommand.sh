...
apt install -y --no-install-recommends wget sudo xz-utils nano bc git python3 bison flex make cppcheck

# Optional: install python tests prerequisites
apt install -y --no-install-recommends python3-pip python3-setuptools python3-psutil python3-six python3-wheel
pip3 install timeout_decorator thrift==0.11.0 osquery pexpect==3.3

# Optional: install RPM packaging prerequisites
apt install -y --no-install-recommends rpm binutils
...
tar xvf /tmp/osquery-toolchain.tar.xz -C /usr/local

# Download and install a newer CMake.
# Afterward, verify that `/usr/local/bin` is in the `PATH` and comes before `/usr/bin`.
wget -O /tmp/cmake.tar.gz https://cmake.org/files/v3.21/cmake-3.21.4-linux-$(uname -m).tar.gz
tar xvf /tmp/cmake.tar.gz -C /usr/local --strip 1
